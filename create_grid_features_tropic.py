...
def main():
    ...
    parser.add_argument('--dataset', type=str, default="tcwv0.5",
    ...
    grid_xy = torch.tensor(np.load(os.path.join(static_dir_path, "pos_feature.npy")
        )) # (2, N_x, N_y)
    ...
    #         dim=1) # (N_grid, 4)
    # grid_features = torch.cat((grid_xy, geopotential), dim=1) # (N_grid, 4)
    grid_features = grid_xy # (N_grid, 2)
